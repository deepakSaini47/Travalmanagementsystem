...
public class Destinations extends JFrame implements Runnable{
    ...
    public Destinations() {
        ...
        caption = new JLabel();
        caption.setBounds(50, 350, 1000, 70);
        caption.setFont(new Font("Tahuma", Font.PLAIN, 30));
        ...
        for (int i = 0; i < 9; i++) {
            ...
            jimage[i] = image[i].getImage().getScaledInstance(650, 450, Image.SCALE_DEFAULT);
            kimage[i] = new ImageIcon(jimage[i]);
            label[i] = new JLabel(kimage[i]);
            label[i].setBounds(0, 0, 650, 450);
            add(label[i]);
        }
